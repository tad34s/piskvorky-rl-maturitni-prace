...
def main():
    ...
    cnnp1 = CNNPLayer(velikost, name="6", to_train=True)
    minimax2 = MinimaxPlayer(depth=6, name="1")
    minimax1 = MinimaxPlayer(depth=3, name="2")
    chatgptmm = MinimaxPlayer2(depth=5, name="3")
    ...
    cnnp1.save_model()
    displaystats(game_record, cnnp1.name, comb)
...
def train(game, player1, player2):
    ...
    # games_won = [1 if x == "CNN Báča" else x for x in games_won]
    # games_won = [-1 if x == "Minim Dreslerová2" else x for x in games_won]
    return games_won
