...
class CNNMemory():
    ...
    def add_game(self, board_state_log, move_log, rewards_log, final_reward):
        if len(self.games) >= self.size:
            self.games.pop(0)
        self.games.append(deepcopy([board_state_log,move_log, rewards_log, final_reward]))

    def __len__(self):
        return len(self.games)

    def get_random_games(self, n):
        if n > len(self):
            n = len(self)
...
class CNNPLayer():
    ...
    def train(self, vysledek, epochs, n_recalls=1):
        ...
        self.train_on_game(epochs)
        games_from_memory = self.memory.get_random_games(n_recalls)
        self.memory.add_game(self.board_state_log, self.move_log, self.reward_log, reward)

        # the last reward is accounted for above
        for game in games_from_memory:
            self.load_values_from_memory(game[0], game[1], game[2], game[3])
            self.train_on_game(epochs)
    ...
    def load_values_from_memory(self, board_state_log, move_log, reward_log, final_reward):
        ...
        self.values_log = []
        self.next_max_log = []
        self.board_state_log = board_state_log
        ...
        self.next_max_log.append(final_reward)
